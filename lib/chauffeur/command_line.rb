...
module Chauffeur
  class CommandLine
    ...
    def initialize(options={})
      ...
      if [@options[:update], @options[:write], @options[:clear]].compact.length > 1
        warn("[fail] Can only update, write or list. Choose one.")
        exit(1)
      end
    end
  ...
  protected
    ...
    def whenever_cron
      ...
      @whenever_cron ||= [comment_open, Chauffeur.cron(@options), comment_close].compact.join("\n") + "\n"
    end
    ...
    def overwrite_config_in_jenkins
      list = Chauffeur::JobConfigList.new(@options)
      list.each do |job_name, job_config|
        Chauffeur::Jenkins.push_config_to_jenkins(job_name, job_config)
      end
    end
    ...
    def comment_base
      "Chauffeur generated tasks for: #{@options[:identifier]}"
    end
